...
function Products({ children }) {
  ...
  const lick = () => {
    ...
    for (i = 0; i < categories.length; i++) {
      if (categories[i].checked) get_category = categories[i].value;
    }
    for (i = 0; i < brands.length; i++) {
      if (brands[i].checked) get_brand = brands[i].value;
    }
    for (i = 0; i < price.length; i++) {
      if (price[i].checked) get_price = price[i].value;
    }
    for (i = 0; i < rating.length; i++) {
      if (rating[i].checked) get_rating = rating[i].value;
    }
    var b = '1';
    window.location.href =
      '/products' + `?${createSearchParams({ f: b, c: get_category, b: get_brand, p: get_price, r: get_rating })}`;
  };
  return (
    <>
      {data !== undefined && (
          ...
          <div className={`${styles['main']} pt-4 pb-4`}>
            <div className={`d-flex`}>
              ...
              <div className="arrow mr-2">
                <Image src="../../../../assets/svg/arrow.svg" alt="" />
              </div>
              <div className={`${styles['direction-item']} mr-2`}>
                <a href="/" className="">
                  Clothings
            ...
            <div className="row pt-4 pb-4 align-items-start">
              <div className="col col-lg-2 pr-3">
                <div className={`${styles['filter-list']}`}>
                  <div className={`${styles['filter-item']} pb-3`}>
                    ...
                    <div className={`${styles['filter-item-title']} pt-3`}>
                      <span>Category</span>
                      <Image className="down" src="../../../../assets/svg/arrow.svg" alt="" />
                    </div>
                    <div className={`${'filter-item-content'}`}>
                      {cat !== undefined &&
                        Object.keys(cat).map((index) => (
                          <div className={`${styles['filter-item-content-item']}`}>
                            <input
                      ...
                      <div className={`${'filter-item-content'}`}>
                        ...
                        <div className={`${styles['filter-item-content-item']}`}>
                          ...
                          <label htmlFor="4-star">
                            ...
                            <svg className="star" data-src="../../../../assets/svg/star.svg"></svg>
                            <svg className="star" data-src="../../../../assets/svg/star.svg"></svg>
                            <svg className="star no-star" data-src="../../../../assets/svg/star.svg"></svg>
                          </label>
                        </div>
                        <div className={`${styles['filter-item-content-item']}`}>
                          <input
                        ...
                        <div className="row">
                          <button type="button" className="btn btn-outline-primary" onClick={lick}>
                            Apply
                          </button>
                        </div>
                      </div>
              ...
              <div className="col pl-3">
                <div className={`${styles['header-products']} bg-w d-flex pt-2 pb-2 pl-3 pr-3 mb-3 rounded border`}>
                  {data.length > 0 ? (
                    <div className={`${styles['about']}`}>{data.length} items found</div>
                  ) : (
                    <div className={`${styles['about']}`}>Không tìm thấy sản phẩm</div>
                  )}
                  <div className={`${styles['grid-products']} d-flex border rounded`}>
                    <div className={`${styles['grid-products-item']} ${styles['active']}`}>
                      <Image src="../../../../assets/svg/mul-grid.svg" alt="" />
                    </div>
                    <div className={`${styles['grid-products-item']}`}>
                      <Image src="../../../../assets/svg/line1.svg" alt="" />
                    </div>
                  </div>
                </div>
                    ...
                    Object.keys(data).map((index) => (
                      <div className={`${styles['products-item']}`} key={index}>
                        <div className={`${styles['product']} row rounded overflow-hidden bg-w pr-3 pl-3 border mb-3`}>
                          <a href={`/product/${data[index].MA_SP[0]}`} className={`${styles['product-image']}`}>
                            <div
                              className="background-img pt-100p"
                              style={{
                                backgroundImage: `url('${data[index] && data[index].HINHANH}')`,
                              }}
                            ></div>
                          </a>
                              ...
                              <div className={`${styles['product-about']}`}>
                                ...
                                <div className={`${styles['product-about']} d-flex`}>
                                  ...
                                  <div className={`${styles['product-sale']}`}>
                                    <span>•</span>
                                    <span>{data[index] && data[index].SL_DA_BAN} orders</span>
                                  </div>
                                </div>
                              </div>
                              <div className={`${styles['product-description']} mt-2 mb-2 overflow-hidden`}>
                                <span className="par-line-2">{data[index] && data[index].MO_TA}</span>
                              </div>
                            </a>
